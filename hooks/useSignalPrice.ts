...
export function useSignalPrice(intervalMs = 3000) {  
  ...
  // Calculate backoff delay using exponential backoff capped at 30s  
  const getBackoffDelay = (failures: number): number => {  
    return Math.min(intervalMs * Math.pow(2, failures), 30000);  
  };  
  
  // Price polling with exponential backoff  
  useEffect(() => {  
    const poll = async () => {  
      try {  
        ...
        // Success: reset failure count and stale flag  
        failureCountRef.current = 0;  
        setIsStale(false);  
      } catch (error) {  
        // Failure: increment count and set stale flag  
  ...
  return { price, flash, relativeTime, isStale };  
}
